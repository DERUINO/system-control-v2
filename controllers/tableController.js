...
class tableController {
    ...
    async getFilteredTables(req, res) {
        try {
            ...
            const tables = await Table.find({ [type]: filteredName }).sort({ createdAt: -1 }).lean()
            const count = await Table.find({ [type]: filteredName }).count().lean()
            res.json({data: tables, status: 200, count: count})
        } catch (e) {
            console.log(e)
            res.status(400).json({message: 'response error'})
    ...
    async getPaginatedTables(req, res) {
        try {
            ...
            const tables = await Table.find().sort({ dateStart: -1 }).skip(page * 50).limit(50).lean()
